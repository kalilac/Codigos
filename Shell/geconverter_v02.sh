...
if [ $1 == "volts" ]; then
	...
	alarm=$($SNMPWALK $2 $3 .1.3.6.1.4.1.10520.2.1.5.1.45.0 | cut -f 3- -d " ")

	checkstringreturns $alarm

elif [ $1 == "alarm02" ]; then

	alarm02=$($SNMPWALK $2 $3 .1.3.6.1.4.1.10520.2.1.5.1.58.0 | cut -f 3- -d " ")

	checkstringreturns $alarm02

elif [ $1 == "alarm03" ]; then

	alarm03=$($SNMPWALK $2 $3 .1.3.6.1.4.1.10520.2.1.5.1.59.0 | cut -f 3- -d " ")

	checkstringreturns $alarm03
	...
	alarm=$($SNMPWALK $2 $3 .1.3.6.1.4.1.10520.2.1.5.1.13.0 | cut -f 3- -d " ")

    checkstringreturns $alarm

elif [ $1 == "batconect" ]; then

	alarm=$($SNMPWALK $2 $3 .1.3.6.1.4.1.10520.2.1.5.13.1.19.0 | awk '{ print $5" "$6 }')
